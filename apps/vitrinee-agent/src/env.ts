...
export function loadRepoEnv(): void {
  const file = resolve(repoRoot(), ".env.vitrinee.local");
  if (existsSync(file)) process.loadEnvFile(file);
}

/** RECEIPT_REGISTRY_ID, else the id committed in deployments/vitrinee-testnet.json. */
export function registryId(): string | undefined {
  const fromEnv = process.env["RECEIPT_REGISTRY_ID"];
  if (fromEnv !== undefined && fromEnv !== "") return fromEnv;
  const file = resolve(repoRoot(), "deployments/vitrinee-testnet.json");
