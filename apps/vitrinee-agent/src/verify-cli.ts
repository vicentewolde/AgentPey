#!/usr/bin/env node
/**
 * `pnpm vitrinee:verify [receipt.jws] [--tamper] [--via-gateway URL]`
 *
 ...
 * Defaults to the last receipt `pnpm vitrinee:buy` saved. `--tamper` lowers the
...
try {
  ...
  process.stderr.write(`✗ no encuentro el recibo en ${path}. Corre primero pnpm vitrinee:buy.\n`);
  process.exit(2);
}
...
if (values["via-gateway"] !== undefined) {
  ...
  if (contractId === undefined) {
    process.stderr.write("✗ no hay RECEIPT_REGISTRY_ID ni deployments/vitrinee-testnet.json con receiptRegistry\n");
    process.exit(2);
  }
