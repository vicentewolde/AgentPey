 ...
 * Reads AGENT_SECRET_KEY from .env.vitrinee.local at the repo root. Saves the receipt
 * to .vitrinee/last-receipt.jws for `pnpm vitrinee:verify`.
 */
...
if (instruction === "") {
  process.stderr.write('uso: pnpm vitrinee:buy -- "compra el hoodie talla M y envíalo a Ñuñoa" [--dry-run] [--no-verify] [--max-usdc 100] [--gateway URL]\n');
  process.exit(2);
}
...
try {
  ...
  if (result.order?.receiptJws != null) {
    ...
    out(`  guardado     ${path} (para pnpm vitrinee:verify)`);
  }
  out("");
  if (values.json) {
