...
## T99 · El comprador de AgentPey ya puede pagarle a Vitrinee · cerrado 2026-09-23, mergeado a `main`
...
### Después del cierre, el mismo día: la dirección ya no le llega al facilitator

El usuario aprobó T99, se mergeó a `main` (`24a2c1b`) y pidió aplicar la
recomendación del punto 1. **Ahora el pedido de pago que viaja al facilitator
ya no lleva el nombre ni la dirección del comprador**, solo la dirección del
checkout del producto. La tienda sigue recibiendo la dirección completa y
creando el pedido con ella: lo único que cambió es qué ve el tercero que mueve
la plata.

Se probó con otra compra real en testnet: el facilitator recibió
`…/checkout/stickers-cordillera`, sin nada más, liquidó igual y el recibo pasó
las tres comprobaciones. Hay un test que falla si la dirección vuelve a
aparecer en lo que se le manda al facilitator; se comprobó que fallaba con el
código anterior.

Lo que **no** cambia: AgentPey guarda la URL completa en el registro de la
compra, que es del partner que mandó esos datos, y la URL pasa por los logs de
acceso en el camino. Para el video del 29, direcciones de prueba igual. Decisión
nueva: [`VT-25`](vitrinee/DECISIONES.md).
...
## T102 · Vitrinee se despliega desde AgentPey, en `vitrinee.agentpey.com` · cerrado 2026-09-23, en vivo
...
### Lo que salió al desplegar

**Vitrinee ya está en vivo** en `vitrinee.agentpey.com`: sirve su manifest, el
catálogo de seis productos reales con sus precios, y responde el cobro (402).
Costó dos tropiezos. El servicio de Render no lee `render.yaml`, así que las
variables públicas hubo que cargarlas a mano (y el gateway ahora exige el
adaptador para arrancarla). Y con Vitrinee dentro, el servicio pasó los 512 MB:
Render descartó dos deploys y dejó corriendo el anterior. Se subió a 1 CPU y
2 GB, 25 USD al mes en vez de 7 (`C-138`, elegido por el usuario); ahora la
memoria ronda el 22 %. El dominio adicional cuesta 0,25 USD al mes.
...
comercios. Ahora Vitrinee publica un directorio de sus tiendas en
`vitrinee.agentpey.com/api/comercios`, y AgentPey confía en la plataforma: toda
tienda que aparezca ahí se vuelve un comercio al que un agente puede pagarle,
siempre que una persona le haya firmado un permiso que la nombre. Nadie tiene
que editar nada ni desplegar para sumar una tienda.

AgentPey no le cree al directorio más de lo necesario. La dirección de cada
tienda la arma él mismo (`https://<tienda>.vitrinee.agentpey.com`). La moneda la
fija su propia lista, no el directorio. Y el cobro solo puede ir a la cuenta que
el directorio publicó para esa tienda: si la tienda pide que se le pague a otra
cuenta, AgentPey rechaza antes de firmar nada. En el permiso firmado, cada
tienda aparece como `vitrinee-<tienda>:<cuenta>`, así se ve de qué plataforma
viene.

En RealOps, el catálogo muestra una sección por cada tienda del directorio. El
"Comprador de la tienda" se contrata desde la tarjeta de un producto, y queda
atado a esa tienda: su permiso nombra esa tienda, su cuenta y sus productos, y
no puede comprar en otra.

### Un problema que apareció al construirlo

La función que arma los términos del pago consultaba la lista fija de comercios
de AgentPey. Una tienda que existe solo gracias al directorio habría fallado
justo en el momento de pagar. Se corrigió en el mismo cambio, y ahí mismo se
agregó el candado de la cuenta de cobro.

### Lo que cambia para quien ya tenía agentes

Los compradores de tienda contratados antes de hoy nombran la tienda con su
nombre viejo (`vitrinee:GC5ZY…`). Siguen apareciendo en RealOps, pero AgentPey
ya no les paga. Para el video hay que contratar uno nuevo desde la tarjeta de
Bazar Cordillera y firmar su permiso. Ya estaba anticipado en `C-142`.

### Lo que tienes que hacer tú, después del merge

1. En Render, borrar `VITRINEE_ROOT_COMERCIO` (la raíz ya no sirve a Bazar
   Cordillera) y `VITRINEE_BASE_URL` (RealOps ya no la lee). Agregar
   `VITRINEE_DIRECTORY_URL=https://vitrinee.agentpey.com/api/comercios`.
2. En RealOps, contratar un comprador desde la tarjeta de un producto de Bazar
   Cordillera, firmar su permiso y comprar algo barato (los stickers). Con eso
   verifico la compra en la red y los tres checks del recibo.

### Evidencia técnica

- Decisión nueva: [`C-145`](DECISIONES.md); `VT-30` marcado como cumplido.
- AgentPey: fila `kind: "platform"` en `venues.json`, `registry.ts`,
  `platforms.ts` (expansión y sus rechazos), `toPaymentTerms` con registro y
  cobro fijado, `tenant-purchase.ts` y `discovery-route.ts` leen el directorio.
- Vitrinee: `GET /api/comercios` en el portal; slug máximo 31.
- RealOps: `storefronts.ts`, `comercio` en el agente (columna nueva, migración
...
de una tienda Jumpseller entra a `vitrinee.agentpey.com` y lo hace solo, en un
par de minutos:

1. **Entra con su wallet.** Aprieta "Conectar Freighter" y firma un mensaje. No
   hay contraseña ni email: la firma prueba que la cuenta es suya, y esa cuenta
   pasa a ser donde se le pagan las ventas. Firmar ese mensaje no mueve plata.
2. **Pega cuatro datos**: el nombre de la tienda, la dirección que quiere para
   los agentes (se propone sola a partir del nombre y avisa si está libre), y el
   login y el token de la API de Jumpseller.
3. **Vitrinee revisa cuatro cosas antes de guardar nada**: que la dirección
   sirva y esté libre; que su wallet pueda recibir USDC; que las credenciales de
   verdad lean el catálogo de su tienda; y que la llave con que se firman sus
   recibos quede creada y con saldo. En pantalla se ve cada una ponerse en
   verde. Si una falla, se ve cuál y qué hacer, y no se guarda nada.
4. **La tienda queda publicada** en `<su-tienda>.vitrinee.agentpey.com`, entra
   al directorio, y AgentPey y RealOps la muestran en menos de un minuto, sin
   deploy.
5. **Ve lo que le compraron**: un panel con sus tiendas y sus pedidos, cada uno
   con su estado y los enlaces al pago en Stellar y al recibo. Nunca los de otra
   persona.

El token de Jumpseller se guarda cifrado y no se vuelve a mostrar nunca, ni
siquiera al dueño. La página está en inglés, con español al lado.

### Lo que decidí dentro de lo que ya estaba decidido

Queda escrito en `VT-31`. Lo más visible: **una misma wallet puede tener varias
tiendas**, así que puedes dar de alta la tienda de prueba con la misma wallet de
Bazar Cordillera y el panel te muestra las dos (no hace falta otra cuenta en
Freighter). La sesión dura 12 horas y se firma con una llave que sale de la
llave maestra que ya cargaste: no hay ningún secreto nuevo que cargar en Render.

### Lo que encontré probándolo

Armé una vista previa local con la página real y dos cosas fallaban: con la
sesión abierta seguía apareciendo el botón de conectar, y el alta se rechazaba
porque el chequeo de origen no contaba el puerto. Los dos se corrigieron antes
de cerrar. Detalle en la evidencia.

### Lo que tienes que hacer tú, después del merge

1. Tener la tienda Jumpseller de prueba con un par de productos, **en pesos
   chilenos** (Vitrinee solo acepta CLP hoy), y a mano su login y su token de la
   API.
2. Tu wallet de Freighter en testnet con USDC agregado (la de Bazar Cordillera
   ya lo tiene).
3. Entrar a `vitrinee.agentpey.com`, conectar Freighter y dar de alta la tienda.
   El token lo pegas tú en la página; yo nunca lo veo. Después verifico en vivo
   que aparezca en el directorio y en RealOps.

### Evidencia técnica

- Decisión nueva: [`VT-31`](vitrinee/DECISIONES.md).
- `packages/vitrinee-gateway/src/platform/`: `wallet-session.ts` (desafío
  SEP-0053 con `verifyStellarMessage` de `@agentpass/core`, cookie firmada),
  `onboarding.ts` (las cuatro pruebas), `stellar-network.ts` (Horizon y
  friendbot), `portal.ts` (rutas `/api/portal/*` y la página), y
  `platform-app.ts` monta el portal solo en el host de la plataforma.
- `packages/vitrinee-core`: seis códigos nuevos de `VitrineeError`, uno por
  rechazo.
...
1. **El guion hablado**, de 5 minutos justos, con los siete tramos que piden los
   organizadores: qué es AgentPey, el problema, la demo, por qué Stellar, qué
   hemos logrado, qué viene y qué pedimos. Cada cifra que se dice tiene su fuente
   anotada, y hay respuestas cortas para las preguntas probables.
2. **Las slides**: ocho, en el estilo negro y amarillo del deck de referencia de
   Stellar, con el guion de cada una en sus notas. La octava es de respaldo: la
   compra real de ayer, por si fallan la demo y el video.
3. **El paso a paso del video de respaldo** (se graba mañana a las 11:00) y de la
   demo en vivo: qué dejar abierto, qué botón apretar en cada segundo y qué no
   mostrar.

La demo en vivo dura 60 segundos y muestra solo el momento clave: un agente ya
contratado compra un pack de stickers, el pago aparece en Stellar y el dueño ve
la venta con su recibo en verde. Contratar el agente y firmar su permiso se hace
antes, no en el escenario (`C-146`).

**Dos cosas que encontré y que cambian lo que se dice en voz alta.** RealOps marca
la compra como "entregado" aunque el pedido todavía no llega a Jumpseller (T101
sigue en `404`), y RealOps no muestra el enlace al recibo: ese enlace solo está
en el panel del dueño. Por eso la demo termina en el panel de Vitrinee, que dice
la verdad ("Pagado, pedido aún no llega a tu tienda"), y el guion evita la
palabra "entregado". No se tocó el código: un cambio hoy redesplegaría Render la
víspera de la demo.

Falta, del usuario: contratar el comprador nuevo y firmar su permiso, grabar el
respaldo y ensayar. Después, Claude Code verifica el pago y el recibo de esa
compra y T106 se cierra.
