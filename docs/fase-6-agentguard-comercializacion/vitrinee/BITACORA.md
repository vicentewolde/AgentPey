...
- `adapters/jumpseller`: lectura de catálogo y creación de pedidos pagados en
  las tres llamadas que la API ofrece ([VT-11](DECISIONES.md)). Filtra lo que
...
- `GET /discovery/resources` en la forma de `@x402/extensions`, documentado
  como espejo local y no como el bazaar oficial ([VT-17](DECISIONES.md)).
...
- Contrato `receipt-registry` desplegado en testnet
  ...
  11 tests Rust, sin admin ni upgrade (VT-3). `pnpm deploy:registry` es
...
- `Idempotency-Key`, una tx = una orden, reserva de stock mientras el pago
  está en vuelo (VT-15). Reintentos de anclaje con estado visible en la orden.
...
- Se agregó un sexto producto barato para las pruebas reales (VT-14). Saldo
...
([VT-10](DECISIONES.md)); `pnpm` anidado agrega un `--` extra a los argumentos
...
- Docs: CONTEXTO, DECISIONES VT-1 a VT-9, esta bitácora, evidencia del día, `CLAUDE.md`.
...
| `soroban-sdk` | 28.0.0 | Verificado hoy: compila para `wasm32v1-none` con `stellar contract build` (ver [evidencia](evidencia/DIA-0.md)). `stellar contract init` aún fija 27; `cargo build` directo ya no sirve ([VT-9](DECISIONES.md)). |
...
[DECISIONES.md § VT-8](DECISIONES.md).
