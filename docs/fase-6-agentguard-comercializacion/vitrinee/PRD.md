...
> **Usuario objetivo** el developer o equipo que ya tiene un agente en operación ([VT-20](DECISIONES.md))
...
HTTP que se conecta a la plataforma de la tienda por su API ([VT-1](DECISIONES.md)),
...
4. **Una prueba que no depende de la tienda.** El recibo es un JWS firmado por
   ...
   del agente verifica **tres cosas por separado** ([VT-13](DECISIONES.md)): que
...
([VT-19](DECISIONES.md)). Vitrinee no genera esa cuenta, no conoce su llave y no
...
Vitrinee **no resuelve la demanda de la tienda** ([VT-20](DECISIONES.md)). No
...
- [ ] El pedido aparece en el panel de la tienda con estado pagado, el producto
      ...
      `additional_information` y en el historial ([VT-11](DECISIONES.md)).
...
- [ ] `pnpm bootstrap` acepta una cuenta `payTo` externa y, en ese modo, **no
      genera cuenta ni escribe secreto alguno** ([VT-19](DECISIONES.md)).
...
|---|---|
| **La plataforma falla después de que el pago se liquidó** | `200` con `status: "paid_unfulfilled"` y registro para cumplimiento manual. Nunca `≥400`: haría creer al agente que no pagó ([VT-10](DECISIONES.md)). |
| **Dos agentes compran la última unidad a la vez** | El segundo recibe `409` **antes** de pagar. El stock queda reservado mientras el pago está en vuelo ([VT-15](DECISIONES.md)). |
...
| **El anclaje falla o el RPC no responde** | El recibo ya es válido; el anclaje reintenta a 2, 5 y 15 s y la orden expone `pending → anchored \| failed` con `lastError`. Se reanuda al arrancar ([VT-5](DECISIONES.md)). |
...
| **El plan de la tienda no permite crear pedidos por API** | El adapter traduce el `403` a un error accionable, no filtra el `403` crudo al agente ([VT-21](DECISIONES.md)). |
| **Un producto tiene stock 0** | Sigue en el manifest, donde el cero es información; **no** aparece en `/discovery/resources`, que es una oferta que el checkout rechazaría ([VT-17](DECISIONES.md)). |
...
| **El comprador declara un `stellarAccount` distinto del que pagó** | Se ignora. El pagador se lee de la transacción firmada o del facilitator; el recibo no se fía del cliente ([VT-10](DECISIONES.md)). |
| **Alguien edita el recibo y lo presenta como válido** | Los tres checks fallan. El `kid` debe coincidir con el `merchantDid`, o cualquiera podría firmar con su propia llave ([VT-13](DECISIONES.md)). |
...
([VT-7](DECISIONES.md)). Ningún `number` representa dinero en ningún punto. Un
...
[VT-2](DECISIONES.md)). No hay oráculo. Lo que se demuestra es que la conversión
...
el handler ([VT-10](DECISIONES.md)). Ninguna orden existe sin pago liquidado. El
...
- **`payTo`** recibe el USDC. La trae el merchant desde su wallet; el gateway no
  tiene su secreto ([VT-19](DECISIONES.md), [VT-4](DECISIONES.md)).
- **Cuenta de firma** emite los recibos y paga los fees de anclaje con su propio
  XLM. No puede mover fondos del merchant ([VT-8](DECISIONES.md)).
...
([VT-3](DECISIONES.md)): un admin podría alterar la prueba que el registro existe
...
el mismo origen ([VT-18](DECISIONES.md)). Dos servicios significan dos despliegues
...
- Descubrimiento, ranking, recomendación y comisión sobre venta referida
  ([VT-20](DECISIONES.md)).
- Oráculo de tipo de cambio ([VT-2](DECISIONES.md)).
- Plugin nativo de plataforma y app oficial con OAuth ([VT-1](DECISIONES.md)).
...
- Aparecer en el bazaar del facilitator declarando la discovery extension en las
  rutas de checkout ([VT-17](DECISIONES.md)).
- Envoltura VC-JWT del recibo ([VT-13](DECISIONES.md)).
