...
([V-8](DECISIONES.md), [V-4](DECISIONES.md)). Un agente que verifica un recibo
...
configuración, no un oráculo ([V-2](DECISIONES.md)). Se publica para que la
...
ejecutable en cadena: no hay escrow ([V-4](DECISIONES.md)). El recibo repite
...
exactamente los que importan ([V-7](DECISIONES.md)). `priceUSDCAtomic` es la
...
compra distinta es `409` ([V-15](DECISIONES.md)).
...
([V-10](DECISIONES.md)). Un agente **no debe** interpretar ese caso como
"no pagué".

`anchor.status: "pending"` es lo normal en la respuesta. El anclaje ocurre
después, con reintentos ([V-5](DECISIONES.md)); el recibo ya es válido y
...
dice el resultado de cada una por separado ([V-13](DECISIONES.md)):
...
local para no aparentar un estándar que no existe ([V-17](DECISIONES.md)).
