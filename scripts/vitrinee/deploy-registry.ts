#!/usr/bin/env node
/**
 * `pnpm vitrinee:deploy-registry` — build, upload, deploy and verify receipt-registry,
 * then record it in deployments/vitrinee-testnet.json and .env.vitrinee.local.
 *
 ...
 * gets no power over the contract: there is no admin (VT-3).
 */
...
import { ReceiptRegistryClient } from "../../packages/vitrinee-anchor/src/index.js";
...
const ENV_PATH = resolve(REPO_ROOT, ".env.vitrinee.local");
const CONTRACTS_DIR = resolve(REPO_ROOT, "contracts/receipt-registry");
const DEPLOYMENT_PATH = resolve(REPO_ROOT, "deployments/vitrinee-testnet.json");
...
async function main(): Promise<void> {
  ...
  if (secret === undefined || secret === "") throw new Error("MERCHANT_SIGNING_SECRET is missing from .env.vitrinee.local — run `pnpm vitrinee:bootstrap` first");
  ...
  if (recorded !== null && !REDEPLOY) {
    ...
    if (live === wasmHash) {
      ...
      await writeEnvFile(ENV_PATH, upsertEnv(content, [{ key: "RECEIPT_REGISTRY_ID", value: recorded.contractId }], "written by pnpm vitrinee:deploy-registry"));
      out();
      return;
    }
    throw new Error(
  ...
  await writeEnvFile(ENV_PATH, upsertEnv(content, [{ key: "RECEIPT_REGISTRY_ID", value: contractId, comment: "receipt-registry on testnet (deployments/vitrinee-testnet.json)" }], "written by pnpm vitrinee:deploy-registry"));
  out(`  wrote      deployments/vitrinee-testnet.json · .env.vitrinee.local RECEIPT_REGISTRY_ID`);
