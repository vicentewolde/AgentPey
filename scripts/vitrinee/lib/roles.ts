 ...
 * Two merchant keys on purpose (docs/fase-6-agentguard-comercializacion/vitrinee/DECISIONES.md, VT-8): the payout account
...
export const ROLES: readonly Role[] = [
  {
    ...
    secretComment:
      "Bootstrap-only: opened the USDC trustline with it. The gateway never reads this variable (VT-8).",
  },
  {
    id: "merchant-signing",
  ...
  },
  {
    ...
    secretComment: "Signs the x402 auth entries in apps/vitrinee-agent.",
  },
];
...
/** Reuses the keypair already in `.env.vitrinee.local`; generates one only when the secret is absent. */
export function resolveKeypair(role: Role, env: Map<string, string>): ResolvedRole {
  ...
  if (secret !== undefined && secret !== "") {
    ...
    try {
      ...
      throw new Error(`${role.secretVar} in .env.vitrinee.local is not a valid Stellar secret seed`);
    }
    const declared = env.get(role.publicVar);
    if (declared !== undefined && declared !== "" && declared !== keypair.publicKey()) {
      throw new Error(
        `${role.publicVar} (${declared}) does not match the key derived from ${role.secretVar} (${keypair.publicKey()}); fix .env.vitrinee.local by hand`,
      );
    }
