/**
 * `deployments/vitrinee-testnet.json` — the one artifact shared by TypeScript and
...
export interface ReceiptRegistryDeployment {
  ...
  wasmHash: string;
  /** Pays for the deploy. Has no power over the contract, which has no admin (VT-3). */
  deployer: string;
...
export function parseDeployment(raw: unknown): Deployment {
  ...
  if (d === null || typeof d !== "object" || d.network !== "testnet" || typeof d.rpcUrl !== "string") {
    throw new Error("deployments/vitrinee-testnet.json is not a testnet deployment file");
  }
  const r = d.receiptRegistry;
  if (r !== null && r !== undefined) {
    if (!/^C[A-Z2-7]{55}$/.test(r.contractId) || !HEX64.test(r.wasmHash) || !HEX64.test(r.deployTxHash)) {
      throw new Error("deployments/vitrinee-testnet.json → receiptRegistry is malformed; fix it by hand, never guess");
    }
  }
